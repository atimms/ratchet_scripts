...
workingDir = "/home/atimms/murat_rnaseq/cerebellum";
setwd(workingDir);

##read in count and metadata
countData1 <- read.table('murat_etoh_0116.cerebellum.htseq_counts.txt', header=T, row.names=1)
colData1 <- read.table('murat_etoh_0116.cerebellum.metadata.txt', header=T, row.names=1)

head(countData1)
head(colData1)
##add to deseq, give countdata and metadata and then design information i.e. info on sample types
dds <- DESeqDataSetFromMatrix(countData = countData1, colData = colData1, design = ~ individual)
dds
...
write.csv(assay(rld), file="murat_etoh_0116.cerebellum.deseq.rlog_counts.csv")
...
pheatmap(sampleDistMatrix,
         ...
         col=colors)
dev.copy2pdf(file='murat_etoh_0116.cerebellum.sample_heatmap.pdf', width = 7, height = 5)

##principal components analysis 
plotPCA(rld, intgroup = c("tissue", "individual"))
ggsave('murat_etoh_0116.cerebellum.sample_pca.pdf') 

##multidimensional scaling plot
mdsData <- data.frame(cmdscale(sampleDistMatrix))
mds <- cbind(mdsData, as.data.frame(colData(rld)))
ggplot(mds, aes(X1,X2,color=tissue,shape=individual)) + geom_point(size=3)
ggsave('murat_etoh_0116.cerebellum.sample_mds.pdf') 
...
dev.copy2pdf(file='murat_etoh_0116.cerebellum.deseq.gene_clustering.pdf', width = 7, height = 5)
...
##better with ggplot
data <- plotCounts(dds, gene="Ddx3y", intgroup=c("tissue","individual"), returnData=TRUE)
ggplot(data, aes(x=tissue, y=count,color=tissue,shape=individual)) +
  scale_y_log10() + 
  geom_point(position=position_jitter(width=.1,height=0), size=3)
ggsave('murat_etoh_0116.cerebellum.Ddx3y.tissue.pdf') 

##save differentially expression results
##sort results by adjusted p-value
resOrdered <- res[order(res$padj),]
head(resOrdered)
##save results as dataframe and take top 100 results, then write csv file
resOrderedDF <- as.data.frame(resOrdered)[1:100,]
write.csv(resOrderedDF, file="results.cerebellum.etoh_vs_ctl.csv")
