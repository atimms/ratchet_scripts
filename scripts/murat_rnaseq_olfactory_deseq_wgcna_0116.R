...
workingDir = "/home/atimms/murat_rnaseq/olfactory";
setwd(workingDir);

##read in count and metadata
countData1 <- read.table('murat_etoh_0116.olfactory.htseq_counts.txt', header=T, row.names=1)
colData1 <- read.table('murat_etoh_0116.olfactory.metadata.txt', header=T, row.names=1)
...
write.csv(assay(rld), file="murat_etoh_0116.olfactory.deseq.rlog_counts.csv")
...
pheatmap(sampleDistMatrix,
         ...
         col=colors)
dev.copy2pdf(file='murat_etoh_0116.olfactory.sample_heatmap.pdf', width = 7, height = 5)

##principal components analysis 
plotPCA(rld, intgroup = c("tissue", "individual"))
ggsave('murat_etoh_0116.olfactory.sample_pca.pdf') 

##multidimensional scaling plot
mdsData <- data.frame(cmdscale(sampleDistMatrix))
mds <- cbind(mdsData, as.data.frame(colData(rld)))
ggplot(mds, aes(X1,X2,color=tissue,shape=individual)) + geom_point(size=3)
ggsave('murat_etoh_0116.olfactory.sample_mds.pdf') 
...
dev.copy2pdf(file='murat_etoh_0116.olfactory.deseq.gene_clustering.pdf', width = 7, height = 5)
...
##better with ggplot
data <- plotCounts(dds, gene="Malat1", intgroup=c("tissue","individual"), returnData=TRUE)
ggplot(data, aes(x=tissue, y=count,color=tissue,shape=individual)) +
  scale_y_log10() + 
  geom_point(position=position_jitter(width=.1,height=0), size=3)
ggsave('murat_etoh_0116.olfactory.Malat1.tissue.pdf') 
...
write.csv(resOrderedDF, file="results.olfactory.etoh_vs_ctl.csv")
