...
workingDir = "/home/atimms/murat_rnaseq/cortex";
setwd(workingDir);

##read in count and metadata
countData1 <- read.table('murat_etoh_0116.cortex.htseq_counts.txt', header=T, row.names=1)
colData1 <- read.table('murat_etoh_0116.cortex.metadata.txt', header=T, row.names=1)
...
write.csv(assay(rld), file="murat_etoh_0116.cortex.deseq.rlog_counts.csv")
...
pheatmap(sampleDistMatrix,
         ...
         col=colors)
dev.copy2pdf(file='murat_etoh_0116.cortex.sample_heatmap.pdf', width = 7, height = 5)

##principal components analysis 
plotPCA(rld, intgroup = c("tissue", "individual"))
ggsave('murat_etoh_0116.cortex.sample_pca.pdf') 

##multidimensional scaling plot
mdsData <- data.frame(cmdscale(sampleDistMatrix))
mds <- cbind(mdsData, as.data.frame(colData(rld)))
ggplot(mds, aes(X1,X2,color=tissue,shape=individual)) + geom_point(size=3)
ggsave('murat_etoh_0116.cortex.sample_mds.pdf') 
...
dev.copy2pdf(file='murat_etoh_0116.cortex.deseq.gene_clustering.pdf', width = 7, height = 5)
...
ggsave('murat_etoh_0116.cortex.Ddx3y.tissue.pdf') 
...
write.csv(resOrderedDF, file="results.cortex.etoh_vs_ctl.csv")
