...
working_dir = '/data/atimms/warman_mut_1217'
...
def run_svaba_paired(t_bam, n_bam, fasta, out_name):
	#svaba run -t $GERMLINE_BAM -p $CORES -L 6 -I -a germline_run -G $REF
	# st_sort_pe = subprocess.Popen([svaba, 'run', '-t', t_bam,'-n', n_bam, '-p', '20', '-I', '-a', out_name, '-G', fasta, '-k', region])
	st_sort_pe = subprocess.Popen([svaba, 'run', '-t', t_bam,'-n', n_bam, '-p', '20', '-I', '-a', out_name, '-G', fasta])
	st_sort_pe.wait()

##call methods, samtools, annovar and format multianno
mut_name = 'mut_mix_1'
bam = mut_name + '.bam'
rd1_fq = mut_name + '.r1.fastq'
rd2_fq = mut_name + '.r2.fastq'
fq_dict = {mut_name: [rd1_fq, rd2_fq]}
st_vcf = mut_name + '.st.vcf.gz'
st_avinputs = [mut_name + '.avinput']

##convert bam to fastq
# convert_bam_fastq_picard(bam, rd1_fq, rd2_fq)
...
##run svaba
run_svaba_single_sample('mut_mix_1.bam', fasta, mut_name + '.svaba_single')

run_svaba_paired('mut_mix_1.bam', 'timon_comb.bwa_mkdup.bam', fasta, mut_name + '.svaba_paired')
