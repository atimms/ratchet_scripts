...
countData1 <- read.table('human_esc_rnaseq_cell_type_0217.star_fc.counts.txt', header=T, row.names=1)
colData1 <- read.table('human_esc_rnaseq_cell_type_0217.star_fc.metadata.txt', header=T, row.names=1)
head(countData1)
head(colData1)
...
pheatmap(sampleDistMatrix,
         ...
         col=colors)
dev.copy2pdf(file='human_esc_rnaseq_cell_type_0217.sample_heatmap.pdf', width = 7, height = 5)

##principal components analysis 
plotPCA(rld, intgroup = c("cell_type"))
ggsave('human_esc_rnaseq_cell_type_0217.sample_pca.pdf') 

##multidimensional scaling plot
mdsData <- data.frame(cmdscale(sampleDistMatrix))
mds <- cbind(mdsData, as.data.frame(colData(rld)))
ggplot(mds, aes(X1,X2,color=cell_type)) + geom_point(size=3)
ggsave('human_esc_rnaseq_cell_type_0217.sample_mds.pdf') 
...
dev.copy2pdf(file='human_esc_rnaseq_cell_type_0217.25_var.gene_clustering.pdf', width = 7, height = 5)
...
dev.copy2pdf(file='human_esc_rnaseq_cell_type_0217.top_50_het_genes.pdf', width = 7, height = 10)
