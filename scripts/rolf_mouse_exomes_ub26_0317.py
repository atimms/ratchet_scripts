...
##working dir
working_dir = '/data/atimms/rolf_mouse_exomes_0317'
...
##files etc
fq_dict = {'mut1': ['mut1.r1.fq', 'mut1.r2.fq'],'mut2': ['mut2.r1.fq', 'mut2.r2.fq'],
		'mut3': ['mut3.r1.fq', 'mut3.r2.fq'],'mut4': ['64.342.01_pe_1.fq.gz', '64.342.01_pe_2.fq.gz']}
post_bwa_bam = '.bwa.bam'
sorted_bam = '.bwa_sorted.bam'
mkdup_bam = '.bwa_mkdup.bam'
bamslist_file = 'bams.list'
st_vcf = 'rolf.0217.vcf.gz'
st_avinputs = ['mut1.avinput', 'mut2.avinput','mut3.avinput', 'mut4.avinput']
...
av_protocol = ['-protocol', 'refGene,rmsk,genomicSuperDups,snp142,generic,generic,generic,generic,generic', '-genericdbfile', 'mut1.avinput,mut2.avinput,mut3.avinput,mut4.avinput,mgp.v4.snps.dbSNP.avinput']
...
window_size = [10000000,5000000,20000000]
step_size = 1000000
info_col = 31
...
def multianno_to_annotated(avinputs): 
	head = ['Chr', 'Start', 'End', 'Ref', 'Alt', 'Func.refGene', 'Gene.refGene', 'GeneDetail.refGene', 'ExonicFunc.refGene', 'AAChange.refGene', 'rmsk', 'genomicSuperDups', 'snp142', 'mut1','mut2','mut3', 'mut4', 'mgp.v4.snps', 'Zygosity', 'Qual', 'Coverage', 'Chr', 'Pos', 'Filter', 'Ref2', 'Alt2','Qual', 'Filter', 'GT_info', 'Format', 'Info']
...
##convert bams to fastq file
# convert_bam_fastq_picard('run0464_lane12_index11=Mut1.bam', 'mut1.r1.fq', 'mut1.r2.fq')
# convert_bam_fastq_picard('run0464_lane12_index12=Mut2.bam', 'mut2.r1.fq', 'mut2.r2.fq')
# convert_bam_fastq_picard('run0464_lane12_index1=Mut3.bam', 'mut3.r1.fq', 'mut3.r2.fq')
...
for sample in samples:
	##exonic_variants
	filtering_annotated.filter(working_dir, "or", sample + '.annotated.xls' , sample + "_1.temp", [col_exon, col_exon], ['==','=='], [exon_definition[0],exon_definition[1]])
	##remove synonymous
	filtering_annotated.filter(working_dir, "and", sample + "_1.temp", sample + "_2.temp", [col_function], ['!='], [syn_definition])
	##remove indels
	filtering_annotated.filter(working_dir, "and", sample + "_2.temp", sample + "_3.temp", [4,5], ['!=','!='], ['-','-'])
	##remove if in dbsnp, sanger, or other mouse line
	filtering_annotated.filter(working_dir, "and", sample + "_3.temp", sample + "_4.temp", [13,18], ['==','=='], ['',''])
	##keep if hom
	filtering_annotated.filter(working_dir, "and", sample + "_4.temp", sample + '.hom_exonic_rare.xls', [zygosity_col], ['=='], ['hom'])
...
for ws in window_size:
	for sample in samples:
		# ##remove if in dbsnp, segdup, or dbsnp or sanger
		filtering_annotated.filter(working_dir, "and", sample + '.annotated.xls', sample + "11.temp", [11,12,13,18], ['==','==','==','=='], ['','','',''])
		##remove indels
