...
dev.copy2pdf(file="./seurat_analysis/d100s/human_scrnaseq_d100s_0620.qc.pdf", width=20)
...
dev.copy2pdf(file="./seurat_analysis/d100s/human_scrnaseq_d100s_0620.pca.pdf", width=20)
ElbowPlot(human, ndims=30)
dev.copy2pdf(file="./seurat_analysis/d100s/human_scrnaseq_d100s_0620.elbow_plot.pdf", width=20)
...
saveRDS(human, file = "./seurat_analysis/d100s/human.rds")
##and load if needed
#human <- readRDS(file = "./seurat_analysis/d100s/human.rds")
...
dev.copy2pdf(file="./seurat_analysis/d100s/human_scrnaseq_d100s_0620.merged.UMAP.res0.4.sample_split.pdf", width = 20)
DimPlot(human_merged, reduction='umap', pt.size=0.1, label = TRUE)
dev.copy2pdf(file="./seurat_analysis/d100s/human_scrnaseq_d100s_0620.merged.UMAP.res0.4.pdf", width=20)
...
write.csv(counts_cluster_sample, file='./seurat_analysis/d100s/human_scrnaseq_d100s_0620.merged.counts_cluster_sample.res0.4.csv')
##graph known markers
...
dev.copy2pdf(file="./seurat_analysis/d100s/human_scrnaseq_d100s_0620.merged.res0.4.known_markers.dotplot.pdf", width = 20)
...
write.csv(human_merged.markers, file='./seurat_analysis/d100s/human_scrnaseq_d100s_0620.merged.markers.res0.4.csv')
...
dev.copy2pdf(file="./seurat_analysis/d100s/human_scrnaseq_d100s_0620.harmony.harmony_plot.pdf", width = 20)
...
dev.copy2pdf(file="./seurat_analysis/d100s/human_scrnaseq_d100s_0620.harmony.UMAP.res0.4.sample_split.pdf", width = 20)
DimPlot(human_harmony, reduction='umap', pt.size=0.1, label = TRUE)
dev.copy2pdf(file="./seurat_analysis/d100s/human_scrnaseq_d100s_0620.harmony.UMAP.res0.4.pdf", width=20)
#Identify the number of cells in each cluster between samples
counts_cluster_sample = table(human_harmony$seurat_clusters, human_harmony$sample)
write.csv(counts_cluster_sample, file='./seurat_analysis/d100s/human_scrnaseq_d100s_0620.harmony.counts_cluster_sample.res0.4.csv')
#Dot plot - the size of the dot = % of cells and color represents the average expression
DotPlot(human_harmony, features = markers) + RotatedAxis()
dev.copy2pdf(file="./seurat_analysis/d100s/human_scrnaseq_d100s_0620.harmony.res0.4.known_markers.dotplot.pdf", width = 20)
#Find all markers that define each cluster
human_harmony.markers <- FindAllMarkers(human_harmony, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
write.csv(human_harmony.markers, file='./seurat_analysis/d100s/human_scrnaseq_d100s_0620.harmony.markers.res0.4.csv')

#Save object to avoid needing to re-run previous computations
saveRDS(human_harmony, file = "./seurat_analysis/d100s/human_harmony.rds")
