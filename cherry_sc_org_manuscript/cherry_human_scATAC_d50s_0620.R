...
dev.copy2pdf(file="./signac_analysis/d50s/human_scatac_d50s_0620.d53.qc.pdf", width=20)
FragmentHistogram(object = d53, group.by = 'nucleosome_group')
dev.copy2pdf(file="./signac_analysis/d50s/human_scatac_d50s_0620.d53.frag_by_ns.pdf", width=20)
TSSPlot(d53, group.by = 'high.tss') + ggtitle("TSS enrichment score") + NoLegend()
dev.copy2pdf(file="./signac_analysis/d50s/human_scatac_d50s_0620.d53.tss_enrichment.pdf", width=20)
...
dev.copy2pdf(file="./signac_analysis/d50s/human_scatac_d50s_0620.UMAP.pdf", width=20)
...
dev.copy2pdf(file="./signac_analysis/d50s/human_scatac_d50s_0620.d59.qc.pdf", width=20)
FragmentHistogram(object = d59, group.by = 'nucleosome_group')
dev.copy2pdf(file="./signac_analysis/d50s/human_scatac_d50s_0620.d59.frag_by_ns.pdf", width=20)
TSSPlot(d59, group.by = 'high.tss') + ggtitle("TSS enrichment score") + NoLegend()
dev.copy2pdf(file="./signac_analysis/d50s/human_scatac_d50s_0620.d59.tss_enrichment.pdf", width=20)
##filter using qc metrics
#pbmc <- subset(pbmc, subset = peak_region_fragments > 3000 & peak_region_fragments < 20000 & pct_reads_in_peaks > 15 & blacklist_ratio < 0.05 & nucleosome_signal < 10 & TSS.enrichment > 2)
...
combined.peaks <- UnifyPeaks(object.list = list(d53_subset,d59_subset), mode = "reduce")
combined.peaks
...
combined <- merge(x = d53_subset, y = list(d59_subset), add.cell.ids = c("d53", "d59"))
...
dev.copy2pdf(file="./signac_analysis/d50s/human_scatac_d50s_0520.merged.res_0.4.UMAP_by_cluster.pdf", width=20)
DimPlot(combined, split.by = 'dataset', pt.size = 0.1, label = TRUE)
dev.copy2pdf(file="./signac_analysis/d50s/human_scatac_d50s_0520.merged.res_0.4.UMAP_split_by_sample.pdf", width=20)

##merged fragments using unix commands, so add to merged object
combined <- SetFragments(combined, "/active/cherry_t/OrgManuscript_SingleCell_Data/human_scATAC/merge_fragments_d50s/fragments.tsv.gz")

# save individual samples with peaks assay and combined object
saveRDS(d53_subset, file = "./signac_analysis/d50s/d53_combPeaks.rds")
saveRDS(d59_subset, file = "./signac_analysis/d50s/d59_combPeaks.rds")
saveRDS(combined, file = "./signac_analysis/d50s/combined.rds")
saveRDS(combined.peaks, file = "./signac_analysis/d50s/combined_peaks.rds")

##load file if needed
#combined <- readRDS(file = "./signac_analysis/d50s/combined.rds")
...
write.csv(combined.markers, file='./signac_analysis/d50s/human_scatac_d50s_0520.merged.markers.csv')
...
closest_genes <- ClosestFeature(regions = open_regions, annotation = gene.ranges,
                                sep = c(':', '-'))
write.csv(closest_genes, file='./signac_analysis/d50s/human_scatac_d50s_0520.merged.marker_gene_ids.csv')
...
fragment.path = "/active/cherry_t/OrgManuscript_SingleCell_Data/human_scATAC/merge_fragments_d50s/fragments.tsv.gz"
...
dev.copy2pdf(file="./signac_analysis/d50s/human_scatac_d50s_0520.merged.known_marker.dotplot.res0.4.pdf", width = 20)
...
dev.copy2pdf(file="./signac_analysis/d50s/human_scatac_d50s_0520.harmony_res0.4.UMAP_by_cluster.pdf", width=20)
DimPlot(hm.integrated, split.by = 'dataset', pt.size = 0.1, label = TRUE)
dev.copy2pdf(file="./signac_analysis/d50s/human_scatac_d50s_0520.harmony_res0.4.UMAP_split_by_sample.pdf", width=20)

##save/load file
saveRDS(hm.integrated, file = "./signac_analysis/d50s/harmony_integrated.rds")
#hm.integrated <- readRDS(file = "./signac_analysis/d50s/harmony_integrated.rds")
...
dev.copy2pdf(file="./signac_analysis/d50s/human_scatac_d50s_0520.harmony.feature_plot1.res0.4.pdf", width=20)
...
write.csv(harmony.markers, file='./signac_analysis/d50s/human_scatac_d50s_0520.harmony.marker.res0.4.csv')
head(harmony.markers)

## get genes closest to marker genes, row names are weird in a few places
##temp way to lead results
#harmony.markers <- read.csv('./signac_analysis/d50s/human_scatac_d50s_0520.harmony.marker.res0.4.csv')
##list of peaks
...
closest_genes <- ClosestFeature(regions = open_regions, annotation = gene.ranges,
                                sep = c(':', '-'))
write.csv(closest_genes, file='./signac_analysis/d50s/human_scatac_d50s_0520.harmony.marker_gene_ids.res0.4.csv')
...
dev.copy2pdf(file="./signac_analysis/d50s/human_scatac_d50s_0520.harmony.known_marker.dotplot.res0.4.pdf", width = 20)
