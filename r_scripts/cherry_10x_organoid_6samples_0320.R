...
library(cowplot)
library(stringr)
setwd('/data/atimms/cherry_10x_6samples_organoid_0320')

###two new samples pointless not used

# Load the 10x datasets for 0319
Mut26.data <- Read10X(data.dir = "/data/atimms/cherry_10x_6samples_organoid_0320/Mut2-6_filtered_feature_bc_matrix")
Mut26 <- CreateSeuratObject(counts = Mut26.data, project = "Mut26", min.cells = 3, min.features = 200)
Mut38.data <- Read10X(data.dir = "/data/atimms/cherry_10x_6samples_organoid_0320/Mut3-8_filtered_feature_bc_matrix")
Mut38 <- CreateSeuratObject(counts = Mut38.data, project = "Mut38", min.cells = 3, min.features = 200)
Mut26_2.data <- Read10X(data.dir = "/data/atimms/cherry_10x_6samples_organoid_0320/KO2-6_2_filtered_feature_bc_matrix")
Mut26_2 <- CreateSeuratObject(counts = Mut26_2.data, project = "Mut26_2", min.cells = 3, min.features = 200)

Wt21.data <- Read10X(data.dir = "/data/atimms/cherry_10x_6samples_organoid_0320/Wt2-1_filtered_feature_bc_matrix")
Wt21 <- CreateSeuratObject(counts = Wt21.data, project = "Wt21", min.cells = 3, min.features = 200)
Wt21_2.data <- Read10X(data.dir = "/data/atimms/cherry_10x_6samples_organoid_0320/WT2-1_2_filtered_feature_bc_matrix")
Wt21_2 <- CreateSeuratObject(counts = Wt21_2.data, project = "Wt21_2", min.cells = 3, min.features = 200)
Wt31.data <- Read10X(data.dir = "/data/atimms/cherry_10x_6samples_organoid_0320/Wt3-1_filtered_feature_bc_matrix")
...
organoid_merge <- merge(Mut26, y = c(Mut26_2, Mut38, Wt21, Wt21_2, Wt31), add.cell.ids = c("Mut26", "Mut26_2", "Mut38", "Wt21", "Wt21_2", "Wt31"), project = "organoid_merge")
...
prop.table(table(Idents(organoid_merge)))
...
dev.copy2pdf(file="organoid_merge_6_0320.qc.pdf", width = 10)
...
plot_grid(p1, p2)
dev.copy2pdf(file="organoid_merge_6_0320.umap_cluster_split.pdf", width = 20)
