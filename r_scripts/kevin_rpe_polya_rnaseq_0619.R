...
countData1 <- read.table('kevin_rpe_polya_0619.star_fc.counts.txt', header=T, row.names=1)
colData1 <- read.table('kevin_rpe_polya_0619.star_fc.metadata.txt', header=T, row.names=1)
head(countData1)
head(colData1)
...
write.csv(count_data, file="kevin_rpe_polya_0619.norm_counts.csv")
...
#write.csv(assay(rld), file="kevin_rpe_polya_0619.deseq.vst_counts.csv")
...
pheatmap(sampleDistMatrix,
         ...
         col=colors)
dev.copy2pdf(file='kevin_rpe_polya_0619.sample_heatmap.pdf', width = 7, height = 5)
##principal components analysis 
plotPCA(rld, intgroup = c("status"))
ggsave('kevin_rpe_polya_0619.status_pca.pdf')
plotPCA(rld, intgroup = c("sample_name"))
ggsave('kevin_rpe_polya_0619.sample_pca.pdf')
plotPCA(rld, intgroup = c("sex"))
ggsave('kevin_rpe_polya_0619.sex_pca.pdf')
plotPCA(rld, intgroup = c("rs1742162"))
ggsave('kevin_rpe_polya_0619.rs1742162_pca.pdf')
...
dev.copy2pdf(file='kevin_rpe_polya_0619.25_var_gene_clustering.pdf', width = 7, height = 5)
...
write.csv(resOrdered2DF, file="kevin_rpe_polya_0619.het_vs_all_wt.csv")
